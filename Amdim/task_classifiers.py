...
def _train(model, optimizer, scheduler, checkpointer, epochs, train_loader,
           test_loader, stat_tracker, log_dir, device, fname):
    '''
    ...
    for epoch in range(next_epoch, epochs):
        ...
        checkpointer.update(epoch + 1, total_updates, classifier=True, fname=fname)


def train_classifiers(model, learning_rate, dataset, train_loader,
                      test_loader, stat_tracker, checkpointer, log_dir, device, fname, n_epochs=30):
