...
def _train(model, optim_inf, scheduler_inf, checkpointer, epochs,
           train_loader, test_loader, stat_tracker, log_dir, device, fname):
    '''
    ...
    for epoch in range(next_epoch, epochs):
        ...
        for _, ((images1, images2), labels, paths) in enumerate(train_loader):
        ...
        checkpointer.update(epoch + 1, total_updates, classifier=False, fname=fname)


def train_self_supervised(model, learning_rate, dataset, train_loader,
                          test_loader, stat_tracker, checkpointer, log_dir, device, fname, n_epochs=50):
    # configure optimizer
